...
@property (atomic,strong) AVCaptureDeviceType captureDeviceType;
...
- (instancetype)initWithMetadataObjectTypes:(NSArray<NSString *> *)metaDataObjectTypes previewView:(UIView *)previewView {
    ...
    if (self) {
        ...
        _captureDeviceType = AVCaptureDeviceTypeBuiltInWideAngleCamera;
...
- (BOOL)startScanningWithCamera:(MTBCamera)camera resultBlock:(void (^)(NSArray<AVMetadataMachineReadableCodeObject *> *codes))resultBlock error:(NSError **)error {
    ...
    _camera = camera;
    self.captureDevice = [self newCaptureDeviceWithCamera:self.camera:_captureDeviceType];
...
- (AVCaptureDevice *)newCaptureDeviceWithCamera:(MTBCamera)camera : (AVCaptureDeviceType) captureType {
    AVCaptureDevice *newCaptureDevice = nil;
    AVCaptureDevicePosition position = [[self class] devicePositionForCamera:camera];
    
    if (@available(iOS 10.0, *)) {
        AVCaptureDevice *device = [AVCaptureDevice defaultDeviceWithDeviceType:captureType
                                                                     mediaType:AVMediaTypeVideo
                                                                      position:position];
...
- (BOOL)hasTorch {
    AVCaptureDevice *captureDevice = [self newCaptureDeviceWithCamera:self.camera:_captureDeviceType];
    NSError *error = nil;
    AVCaptureDeviceInput *input = [AVCaptureDeviceInput deviceInputWithDevice:captureDevice error:&error];
...
- (BOOL)setCamera:(MTBCamera)camera error:(NSError **)error {
    ...
    AVCaptureDevice *captureDevice = [self newCaptureDeviceWithCamera:camera:_captureDeviceType];
    AVCaptureDeviceInput *input = [AVCaptureDeviceInput deviceInputWithDevice:captureDevice error:error];
...
- (void) setZoom:(CGFloat)zoomRate{
    self.captureDevice = [self newCaptureDeviceWithCamera:self.camera:self.captureDeviceType];
    self.captureDevice.videoZoomFactor = zoomRate;
}
