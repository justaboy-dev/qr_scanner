...
public class QRView:NSObject,FlutterPlatformView {
    ...
    @objc func handlePinch(_ sender: UIPinchGestureRecognizer) {
        var currentScale: CGFloat = scanner?.getCaptureDevice().videoZoomFactor ?? 1.0;
        let maxScale = scanner?.getCaptureDevice().maxAvailableVideoZoomFactor ?? 1.0
        switch sender.state {
        case .began:
            break;
        case .changed:
            let scaleValue = currentScale + (sender.scale - 1) * 0.5;
            NSLog("%f", scaleValue);
            currentScale = max(1.0, min(scaleValue, maxScale));
            self.setZoom(currentScale);
        case .possible:
            break
        case .ended:
            break
    ...
    func setZoom(_ zoomRate: CGFloat){
        do{
            let captureDevice = scanner?.getCaptureDevice()
            try
            captureDevice?.lockForConfiguration();
            scanner?.setZoom(zoomRate);
