...
class QRView(messenger: BinaryMessenger, id: Int, private val params: HashMap<String, Any>) :
    ...
    class ScaleListener(barcodeView: BarcodeView) :ScaleGestureDetector.OnScaleGestureListener {
        private var _previousZoom:Float = 1F;
        ...
        override fun onScale(p0: ScaleGestureDetector?): Boolean {
            val isZoomIn: Boolean = _previousZoom < _previousZoom * p0!!.scaleFactor
            barcodeView.changeCameraParameters {
                parameters ->
                val maxScale: Int = parameters.maxZoom
                var currentScale: Int = parameters.zoom
                currentScale = if(isZoomIn){
                    1.coerceAtLeast((currentScale + 1).coerceAtMost(maxScale));
                }else {
                    1.coerceAtLeast((currentScale - 1).coerceAtMost(maxScale));
                }
                parameters.zoom = currentScale;
                parameters
            }
            return true
        }
    ...
    private fun startScan(arguments: List<Int>?, result: MethodChannel.Result) {
        ...
        detector = ScaleGestureDetector(Shared.activity!!.baseContext,barcodeView?.let { ScaleListener(barcodeView = it) })

        barcodeView!!.setOnTouchListener{
                _,e ->
